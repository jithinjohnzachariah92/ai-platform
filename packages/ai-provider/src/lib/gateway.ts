...
import { assertSafeInput, scrubSecrets } from '@jz92/ai-core';
...
async function execute<T>(
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    try {
      ...
      const wrapped =
        err instanceof AIProviderError ? err : wrapError(err, config.provider);
  ...
  emitEvent({
    ...
    durationMs: Date.now() - start,
    error: lastError
      ? { code: lastError.code, message: scrubSecrets(lastError.message) }
      : { code: 'UNKNOWN', message: 'exhausted retries' },
    correlationId: traceId,
  });
  throw lastError;
}
...
function logUsageBox<T>(config: ProviderConfig, response: AIResponse<T>): void {
  ...
  lines.push(
    line(
      'provider',
      `\x1b[36m${config.provider}\x1b[0m \x1b[2m(${config.env})\x1b[0m`,
    ),
  );
  ...
  if (u) {
    lines.push(
      line(
        'tokens',
        `in: \x1b[33m${u.inputTokens}\x1b[0m  out: \x1b[33m${u.outputTokens}\x1b[0m`,
      ),
    );
    if (u.cacheReadTokens > 0) {
      ...
      lines.push(
        line(
          'cache read',
          `\x1b[32m${u.cacheReadTokens} tokens (${pct}% of input)\x1b[0m`,
        ),
      );
    }
    if (u.cacheCreationTokens > 0) {
      lines.push(
        line('cache write', `\x1b[35m${u.cacheCreationTokens} tokens\x1b[0m`),
      );
    }
  }
  lines.push(bar('└'));
...
function logCacheHitBox(config: ProviderConfig, stats: CacheStats): void {
  ...
  if (!isDev && !forceLog) return;
  console.log(
    `\x1b[2m[ai-provider]\x1b[0m \x1b[32mapp-cache hit\x1b[0m — hitRate ${stats.hitRate}, size ${stats.size}`,
  );
}

// ─── Vision + structured output ────────────────────────────────────────────────

export const generateStructuredFromImage = async <T>(options: {
  ...
  const config = resolveVisionProvider();
  ...
  return buildResponse(result.output as T, result.usage, config, undefined);
};
