...
export const printTraceSummary = (traceId: string): void => {
  ...
  const sorted = [...buf.events].sort((a, b) =>
  ...
  for (const node of STAGE_HIERARCHY) {
    ...
    for (const e of events) {
      const children: { label: string; ms: number }[] = [];
      for (const childKey of node.children ?? []) {
        ...
        for (const ce of childEvents) {
          children.push({
            label: CHILD_LABELS[childKey] ?? childKey,
            ms: ce.durationMs ?? 0,
          });
        }
      }

      // Append domain when the event carries one — retrieval/store events
      // from @jz92/retrieval do; most others don't and this is a no-op for them.
     const domain = (e as any).domain;
      const label = domain ? `${node.label} (${domain})` : node.label;

      rows.push({ label, ms: e.durationMs ?? 0, indent: false });

      for (const child of children) {
        rows.push({ label: child.label, ms: child.ms, indent: true });
      }
    }
  }
  ...
  for (const [key, events] of byKey) {
    ...
    for (const e of events) {
      rows.push({
        label: activeConfig.labels?.[key] ?? key,
        ms: e.durationMs ?? 0,
        indent: false,
      });
    }
  ...
  const topLevelTotal = rows
    .filter((r) => !r.indent)
    .reduce((sum, r) => sum + r.ms, 0);

  const width = 47;
  const bar = (c: string) =>
    `[trace] ${c}${'─'.repeat(width)}${c === '┌' ? '┐' : '┘'}`;
  ...
  for (const r of rows) {
    const pct =
      !r.indent && topLevelTotal > 0
        ? ` (${Math.round((r.ms / topLevelTotal) * 100)}%)`
        : '';
